...
# DB Connection
db = MemoryRepository()
# db = GCloudRepository()backendflask.
...
class Movie(Resource):
    ...
    def put(self, movieID: str) -> str:
        ...
        response['successful'] = True if db.update_movie(
            Movie(
                movieID=args['movieID'],
    ...
    def delete(self, movieID: str) -> str:
        response = {
            "successful": False,
        }
        response['successful'] = True if db.delete_movie(
            movieID=movieID) else False
        if response['successful']:
            return make_response(jsonify(response), 200)
        else:
            return make_response(jsonify(response), 404)
